...
def encrypt(public_key, plaintext, label=b''):
    ...
    X = xor(db, mask_generation(s, k - 32 - 1))
    Y = xor(s, mask_generation(X, 32))

    m = bytes2int(b'\x00' + Y + X)
    c = pow(m, e, N)
    return int2bytes(c)
...
def decrypt(private_key, ciphertext, label=b''):
    ...
    masked_r, masked_X = plaintext[-(k - 1): -(k - 32 - 1)], plaintext[-(k - 32 - 1):]

    seed = xor(masked_r, mask_generation(masked_X, 32))
    db = xor(masked_X, mask_generation(seed, k - 32 - 1))
